# colocacion de creditos guide installation
...
conda activate colocacion_de_creditos
```
...
mamba activate colocacion_de_creditos
```
or 
...
To move beyond notebook prototyping, all reusable code should go into the `colocacion_de_creditos/` folder package. To use that package inside your project, install the project's module in editable mode, so you can edit files in the `colocacion_de_creditos` folder and use the modules inside your notebooks :
...
```
This will install the project module in "editable" mode so that any changes you make to the code in the colocacion_de_creditos/ folder will be reflected immediately without needing to reinstall.
...
```python
from colocacion_de_creditos.utils.paths import data_raw_dir
